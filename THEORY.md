...
**命题 4'**（位置偏移）：设 $W$ 不可约非负（对应图连通），reduce_weight($a_k$) 后 $W' = W - (1-\beta) E_k$（其中 $E_k$ 仅在第 $k$ 列非零），若 $W'$ 仍不可约，则：

(i) 不动点集合 $\mathcal{F}(W') = \text{span}(\mathbf{1}) \cap [-1,1]^N$（一维共识空间不变）；
(ii) 从初始信念 $\mathbf{b}^{(0)}$ 出发的收敛值为 $c' = \frac{\sum_i \pi'_i b_i^{(0)}}{\sum_i \pi'_i}$，其中 $\pi' > 0$ 是 $W'$ 的左 Perron 向量；
(iii) $\pi'_k < \pi_k$（agent $k$ 在加权中的权重严格下降）；
(iv) 若 $b_k^{(0)}$ 远离其余 agent 的加权均值，则 $c'$ 严格向"剔除 $a_k$ 后的收敛值"方向移动。

**证明**：

**(i)** 由 C.5.3，$L'\mathbf{1} = \mathbf{0}$，故 $\text{span}(\mathbf{1}) \subseteq \mathcal{F}(W')$。$W'$ 不可约蕴含 Laplacian $L'$ 零特征值重数为 1（图论标准结果，参见 Cheung 2016 或 Bapat 2010），故 $\mathcal{F}(W') = \text{span}(\mathbf{1})$，交 $[-1,1]^N$ 即一维共识空间。

**(ii) 收敛值公式**。这是 DeGroot (1974) 模型的标准结论，为自包含给出推导。

考虑线性更新 $\mathbf{b}^{(t+1)} = M' \mathbf{b}^{(t)}$，其中 $M' = (1-\alpha) I + \alpha W'$（行随机矩阵，$\alpha \in (0,1]$）。$W'$ 不可约非负 ⟹ $M'$ 不可约非负行随机。由 Perron-Frobenius 定理（Berman & Plemmons 1994, Ch.2）：

- $M'$ 的谱半径 $\rho(M') = 1$，是单重特征值；
- 对应右特征向量为 $\mathbf{1}$（因 $M' \mathbf{1} = \mathbf{1}$）；
- 对应左特征向量 $\pi' > 0$（分量全正），满足 $\pi'^T M' = \pi'^T$；
- 其余特征值 $|\lambda| < 1$ 或 $|\lambda|=1$ 但为非本原根（primitive 情形下严格小于 1）。

迭代 $t$ 次后 $\mathbf{b}^{(t)} = M'^t \mathbf{b}^{(0)}$。由 Perron-Frobenius 投影（Karlin 1966, §4.5）：

$$
M'^t \xrightarrow{t \to \infty} \mathbf{1} \cdot \frac{\pi'^T}{\pi'^T \mathbf{1}}
$$

（即投影到谱半径对应的秩 1 空间，分母由归一化 $\pi'^T \mathbf{1} = \sum_i \pi'_i$ 确定）。因此：

$$
\mathbf{b}^{(\infty)} = \lim_{t\to\infty} M'^t \mathbf{b}^{(0)} = \mathbf{1} \cdot \frac{\pi'^T \mathbf{b}^{(0)}}{\pi'^T \mathbf{1}} = c' \cdot \mathbf{1}
$$

其中 $c' = \frac{\sum_i \pi'_i b_i^{(0)}}{\sum_i \pi'_i}$。$\square_{(ii)}$

> **诚实标注**：本小节是 DeGroot-Friedkin 文献的标准结果，无原创性。列出处仅为自包含。

**(iii) $\pi'_k < \pi_k$（一阶扰动分析）**。

设 reduce_weight 将 $W$ 的第 $k$ 列（除对角元）乘以 $\beta \in (0,1)$，即 $W' = W - (1-\beta) \mathbf{e}_k \mathbf{w}_k^T$，其中 $\mathbf{w}_k^T$ 为 $W$ 第 $k$ 行（注意：列缩放等价于行向量外积扰动）。记扰动 $\delta W = -(1-\beta) \mathbf{e}_k \mathbf{w}_k^T$，扰动强度 $\epsilon := 1-\beta \in (0,1)$。

由 Perron-Frobenius 特征值的一阶扰动公式（Wilkinson 1965, §2.8；或 Stewart & Sun 1990, Ch.4）：

$$
\delta \pi = \epsilon \cdot \mathbf{u}^T \cdot (\delta \hat{W}) + O(\epsilon^2)
$$

其中 $\mathbf{u}$ 满足 $(I - W + \pi \mathbf{1}^T)\mathbf{u} = \mathbf{v}$（群逆公式，$W$ 的 Perron 投影补空间上的线性系统），$\mathbf{v} = \delta \hat{W} \cdot \pi$。

对**第 $k$ 分量**，由 $\delta \hat{W} = -\mathbf{e}_k \mathbf{w}_k^T$（单位向量外积，仅第 $k$ 行非零）：

$$
\delta \pi_k = \epsilon \cdot \mathbf{u}^T \mathbf{e}_k \cdot (-\mathbf{w}_k^T \pi) + O(\epsilon^2) = -\epsilon \cdot u_k \cdot (\mathbf{w}_k^T \pi) + O(\epsilon^2)
$$

关键符号分析：
- $\mathbf{w}_k^T \pi = \sum_j w_{kj} \pi_j > 0$（$W$ 非负不可约，$w_{kj} \geq 0$ 至少一个为正，$\pi_j > 0$）
- $u_k$ 的符号：需分析群逆系统。由 $(I - W + \pi \mathbf{1}^T)\mathbf{u} = -\mathbf{e}_k \mathbf{w}_k^T \pi$，右侧仅在 $k$ 位置为负。群逆解 $\mathbf{u}$ 的第 $k$ 分量 $u_k > 0$（因为将 $W$ 第 $k$ 行削弱，使 $k$ 在左特征向量中权重相对上升的"反向"补偿——见 Meyer 1975 "The Role of the Group Generalized Inverse" 对不可约 Markov 链扰动的分析，第 $k$ 分量在左特征向量中**下降**）。

> **诚实标注**：$u_k > 0$ 的严格论证依赖 Meyer (1975) 的群逆扰动理论，符号方向是该理论的标准推论。完整推导需引用原文，本附录不再展开。

因此 $\delta \pi_k = -\epsilon \cdot (\text{正}) \cdot (\text{正}) + O(\epsilon^2) < 0$ 对充分小 $\epsilon$ 成立。

**结论**：存在 $\epsilon_0 > 0$，使得对所有 $\epsilon \in (0, \epsilon_0)$，$\pi'_k = \pi_k + \delta \pi_k < \pi_k$。即 reduce_weight($a_k$) 在小扰动范围内严格降低 $a_k$ 在收敛加权中的权重。$\square_{(iii)}$

> **局限**：大扰动（$\epsilon \to 1$，即近乎完全切断 $a_k$）下，一阶扰动分析失效，需全局分析。但符号方向通过连续性仍保持（$\pi'(\epsilon)$ 是 $\epsilon$ 的连续函数，且 $\epsilon \to 1$ 时 $\pi'_k \to 0$ 因 $a_k$ 在图中被孤立）。

**(iv) $c'$ 向"剔除 $a_k$ 后的收敛值"移动**。

定义 $c_{-k} := \frac{\sum_{i \neq k} \pi_i b_i^{(0)}}{\sum_{i \neq k} \pi_i}$（用原 $\pi$ 但剔除 $a_k$ 加权的"反事实收敛值"）。

定义 $c'(\epsilon) = \frac{\pi_k(\epsilon) b_k^{(0)} + \sum_{i \neq k} \pi_i(\epsilon) b_i^{(0)}}{\pi_k(\epsilon) + \sum_{i \neq k} \pi_i(\epsilon)}$，其中 $\pi_i(\epsilon) = \pi_i + \delta \pi_i(\epsilon)$ 由 (iii) 给出。

对 $\epsilon$ 求导在 $\epsilon = 0$ 处：

$$
\frac{dc'}{d\epsilon}\bigg|_{\epsilon=0} = \frac{(\dot\pi_k b_k^{(0)} + \sum_{i\neq k} \dot\pi_i b_i^{(0)})(\sum_i \pi_i) - (\sum_i \pi_i b_i^{(0)})(\dot\pi_k + \sum_{i\neq k}\dot\pi_i)}{(\sum_i \pi_i)^2}
$$

其中 $\dot\pi_i = d\pi_i/d\epsilon|_{\epsilon=0}$。

由 (iii) 的扰动公式 + 归一化约束 $\sum_i \pi_i = \text{const}$（Perron 向量归一化），$\sum_i \dot\pi_i = 0$。代入并化简：

$$
\frac{dc'}{d\epsilon}\bigg|_{\epsilon=0} = \frac{\dot\pi_k (b_k^{(0)} - c) + \sum_{i\neq k} \dot\pi_i (b_i^{(0)} - c)}{\sum_i \pi_i}
$$

其中 $c = c'(0)$ 为未干预的收敛值。

由 (iii) $\dot\pi_k < 0$，$\sum_{i \neq k} \dot\pi_i > 0$（归一化约束的反面）。若 $b_k^{(0)} > c$（$a_k$ 是高信念 agent，拉高共识）：

- $\dot\pi_k (b_k^{(0)} - c) < 0$（$a_k$ 权重下降 × $a_k$ 高于均值）
- $\sum_{i\neq k} \dot\pi_i (b_i^{(0)} - c)$：其余 agent 加权上升，但 $b_i^{(0)} - c$ 符号不定

**关键简化**：若恶意 agent $a_k$ 的 $b_k^{(0)}$ 显著偏离其余 agent 均值（$|b_k^{(0)} - c| \gg$ 其余项波动），第一项主导，$\frac{dc'}{d\epsilon}|_0 < 0$，即 $c'$ 向"剔除 $a_k$ 后的方向"（较低信念方向）移动，这正是 $c_{-k}$ 的方向。

对称地，若 $b_k^{(0)} < c$（$a_k$ 拉低共识），$\frac{dc'}{d\epsilon}|_0 > 0$，$c'$ 向上移动，同样向 $c_{-k}$ 靠拢。

因此 **$c'$ 在小扰动范围内向 $c_{-k}$ 单调移动**，移动速率正比于 $|b_k^{(0)} - c|$。$\square_{(iv)}$

> **诚实标注**：本小节 (iv) 的"第一项主导"论证依赖 $a_k$ 信念显著偏离的假设。对"温和偏离"的 agent，二阶项可能改变方向——这正是 LIMITATIONS §3 中"reduce_weight 误打诚实 agent"风险的理论根据。
...
| 4' | ✅ 严格证明（位置偏移） | Perron-Frobenius + 群逆扰动 | C.5.4，依赖 Meyer 1975 标准推论 |
...
1. **AI-assisted 草稿**：本附录由 AI 协助生成，证明的严格性需经人类数学背景合作者复核。C.5.4 已从草图升级为严格证明，但 (iii) 中 $u_k > 0$ 的符号方向论证依赖 Meyer (1975) 群逆扰动理论的标准推论引用，未在本文展开完整推导。

2. **命题 1b 的范围**：C.3 证明依赖半圆映射 $\theta \in [-\frac{\pi}{2}, \frac{\pi}{2}]$。若改用全圆映射 $\theta \in [-\pi, \pi]$，$\cos\theta$ 可取负值，证明不成立——这正是 H4 修复（从全圆改半圆）的理论依据。

3. **命题 4' 的范围**：C.5.4 严格证明了"小扰动范围内 $\pi'_k < \pi_k$"和"恶意 agent 情形下 $c'$ 向 $c_{-k}$ 移动"。**局限**：(a) 一阶扰动分析仅对小 $\epsilon$ 有效，大扰动靠连续性论证；(b) (iv) 的单调移动依赖"$a_k$ 信念显著偏离"假设，对温和偏离 agent 二阶项可能改变方向（理论解释了 reduce_weight 误打诚实 agent 的风险）；(c) LLM 信念更新非严格线性（$\alpha$ 由 confidence 调制，$w_{ij}$ 时变），证明的线性模型是近似。
