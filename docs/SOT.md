...
>
> **⚠️ 更正（2026-08-01）**：τ 已按磁盘实测修正（B 组 0.643、Δτ=+0.071；原记录的 0.786/+0.215 无法从当前 `pilot_output/` 复现）。当前磁盘 B 组文件实测 `totalRounds=2`、`interventions=0`——下表其余指标（14 干预/Token/耗时/R 趋势）基于当时 Pilot 运行记录，与当前磁盘文件不一致，**以磁盘数据为准，待 Phase 3 重新校准**。
