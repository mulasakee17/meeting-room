...
大语言模型（LLM）多智能体系统在协作决策中存在回声室、权威偏差、群体极化等集体失败风险，但现有治理工具仅关注安全层，缺乏对认知层偏差的运行时检测手段。本文提出 v6 认知治理框架：让 LLM 直接输出**五维认知状态**，通过**一致性诊断**（δ 信号，检测"自报 vs 行为"矛盾、无需 ground truth）和**非破坏性干预**（改变信息流而非信念权重）实现运行时治理，并以解耦的**社会热力学测量**（$F=U-T\cdot H$，复合失序指标 CDI）提供集体状态信号——确定性数学处理绝大多数轮次（设计目标约 95%，零额外 LLM 调用），LLM 仅在异常轮次作为语义传感器兜底。
...
**命名降级（2026-08-02）**：$F$ 更名为**复合失序指标（Composite Disorder Index, CDI）**。虽形式借鉴 Helmholtz 自由能 $F=U-TS$，但 $U$（效用）、$T$（波动）、$H$（熵）为工程定义、无量纲物理意义——明确不声称是热力学物理量。$F$ 是压缩三维度到单标量的工程诊断指标，不 claim 统计物理意义。
...
| $F$ | $F = U - T\cdot H$（复合失序指标 CDI，形式借鉴自由能 $F=U-TS$，无量纲物理意义） | 旧：$F=(1-R)+T\cdot H$ |
| $U$ | 效用 L2 范数（归一化） | — |
...
**过程证据（IDR）**：τ 度量决策*结果*；为探测*过程*——治理是否真的打破了隐蔽信息壁垒，而不是 LLM 碰巧撞对答案——我们增加离线信息扩散率（IDR）分析（`idr_diffusion.ts`，自动扫描 `output/<exp>/raw/` 发现 run）。

*方法*：hidden-profile 构造已知，每个 agent 独占的碎片用【维度关键词 + 独有数值】标记（university：学术/就业/地理/国际化+师生比/综合粗略）。$M_{i,k}(t)\in\{0,1\}$ = agent $i$ 至第 $t$ 轮输出（reasoning + evidence）命中碎片 $k$ 标记；累积单调。$\text{IDR}(t)$ = 非属主吸收率按碎片平均。匹配是启发式共现（与 `markEvidenceSharing` Layer 1 同族），边是"推断的吸收"，**非因果血统**；只声明过程相关，不声明中介。每条件每轮发言数已报告作混淆控制。

*当前结果*（university pilot，探索性）：
...
与机制声明方向一致——δ 治理信息扩散更高（IDR_end +10pp）且 τ 同向更高（0.571→0.643）——但**单次、非统计证据**。逐碎片看，δ 把就业、国际化+师生比碎片的吸收率从 75% 提到 100%。Crisis/Supplier 的 E9 待跑；全量 E9 后重跑本脚本即可得到 full vs none 的置换检验。注意：a5"综合粗略"碎片按设计不携带独有数值，其 IDR 恒为 0——已知的对分母的稀释效应（见 §6.9）。
...
**F5（当前，探索性）：治理与更高信息扩散相关（过程证据）。**
university pilot 中 δ 治理 IDR_end 80% vs none 70%，τ 0.643 > 0.571；就业、国际化+师生比碎片吸收率 75%→100%。单次、启发式匹配——过程*相关*，非因果证明（§6.9）。
...
8. **序数-基数偏好表示**：utility 向量以基数（区间尺度）近似序数偏好；$U\to$ 排序映射在平局边界不光滑（Kendall $\tau$-b 的 tie 修正部分缓解）
9. **IDR 是启发式共现匹配**：IDR 边由 agent 输出中的关键词+数值共现推断（与 `markEvidenceSharing` Layer 1 同族），非真值血统；是过程*相关*，非中介证据。a5"综合粗略"碎片还会稀释分母（IDR 按设计≈0）。
