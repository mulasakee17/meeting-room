...
> 本节记录通过 `experiments/v2/analyze_thermo_correlation.ts` 对现有 259 个样本（8 个数据源）进行统计分析后发现的两项根本性局限。这两项局限动摇了基于 scalar `belief` 的热力学度量的可靠性，但不影响 5 维认知状态（posthoc 模式从 itemBeliefs 反推）的已有结论。
