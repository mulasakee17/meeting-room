...
> **多Agent共识动力学的通用计算模型** — 信息不对称 × 社交互动 × 数学共识测量
>
> v9.7 · TypeScript 全栈 · ~20,000 行 · domain-agnostic 核心架构
>
> 作者：贺孟元 | MIT License
...
SwarmAlpha 是一个**多Agent共识动力学的通用计算模型**。它的核心问题是：
...
```
信息输入 → 维度提取 → 视角解读 → 社交互动 → 共识聚合 → 诊断与反事实
```

每个阶段都是纯函数（给定相同输入，永远产生相同输出）。LLM 仅在维度提取阶段被调用一次。

### 设计原则

- **信息盲区是一等公民**：认知主体异质性来自信息维度可见性差异，不是 prompt 差异
- **数学可审计**：从维度 → 信念 → 共识的每一步都有明确的数学公式
- **LLM 最小化**：LLM 只做它擅长的事（语义分解），其余全部纯数学
- **类型安全全栈**：TypeScript 编译时验证从 API 契约到 UI 渲染的整个数据流
- **通用化架构**：核心机制（信息盲区、非线性共识、Kuramoto同步、反事实推断、三维度量）全部是 domain-agnostic 的

### 架构通用性

当前实现以金融市场为验证场景，但核心架构可适配任何多Agent协作场景：

| 金融场景 | 通用化概念 | 适用场景 |
|---------|----------|---------|
| 正交五因子 | 信息维度 | 任何领域的信息结构化 |
| Agent角色 | 认知视角 | 任何多Agent系统的信息不对称 |
| 共识聚合 | 决策融合 | 任何多Agent系统的集体决策 |
| Kuramoto同步化 | 群体协调性度量 | 任何多Agent系统的状态监测 |
| 反事实推断 | 假设场景推演 | 任何多Agent系统的决策验证 |
...
- **没有实时联网搜索**：维度提取仅基于输入文本 + 市场数据。不会搜索相关信息获取额外上下文。
- **Mock 模式关键词有限**：中文约 30 个关键词，英文约 20 个。复杂语义场景依赖 LLM。
- **认知主体数量固定为 9**：架构支持参数化生成更多主体，但当前为固定配置。
- **单市场聚焦**：当前仅美股相关数据源。缺乏其他市场支持。
- **尚未通用化**：核心架构是 domain-agnostic 的，但当前实现仍以金融场景为主。
- **单人项目**：缺乏团队、缺乏商务验证、缺乏企业客户反馈。

### 路线图

**短期（v9.8-v10.0）**
- [ ] 通用化核心抽象：FinancialFactor → InformationDimension，AgentDefinition → PerspectiveDefinition
- [ ] 构建非金融 Demo（日程协商、决策支持等）
- [ ] 发布开源版本（Apache 2.0）
- [ ] 通用共识 API 设计（支持任意领域的信息维度和认知视角）

**中期（v10.x）**
- [ ] 参数化认知主体生成（不限于固定角色）
- [ ] Agent 自主信息获取（主动搜索、交叉验证、信息可靠性判断）
- [ ] 随机 Agent 生命周期（失败/退出/新进入）
- [ ] 多领域模板库（金融、日程、医疗、政策等）
